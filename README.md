...
5. 在程序中选择游戏窗口：
   ...
   - 在 `Window list` 中选择你的游戏窗口。
   - 也可以在 `Window title` 中手动填写游戏窗口标题的一部分。
   - `OCR`：选择 `auto` 或 `tesseract`。
   - `Translator`：选择 `openai`、`deepseek`、`grok`、`argos` 或 `libretranslate`。
   - 点击 `Place beside`。
   - 点击 `Select area`，在游戏窗口上拖拽框选字幕区域。
   - 点击 `Start`。

如果识别区域不准确，可以重新点击 `Select area` 框选，或者手动调整窗口底部的字幕区域裁剪比例。
...
适合已经在使用 OpenAI API，并希望使用 GPT 或 Codex 类模型进行在线翻译的用户。
...
`Select area` 的用法参考了常见 OCR 翻译器的区域选择体验：先选中游戏窗口，然后在窗口上覆盖一层半透明遮罩，用鼠标拖出字幕框。

- 左键拖拽：选择 OCR 字幕区域
- 松开左键：保存区域
- 右键或 `Esc`：取消选择

保存后，程序会自动把选择框转换成相对于游戏窗口的 `Left / Top / Right / Bottom` 比例。之后即使游戏窗口移动，只要窗口大小不变，区域仍然有效。
...
5. Select the game window in the app:
   ...
   - Select your game in `Window list`.
   - Or manually set `Window title` to part of your game window title.
   - Set `OCR` to `auto` or `tesseract`.
   - Set `Translator` to `openai`, `deepseek`, `grok`, `argos`, or `libretranslate`.
   - Click `Place beside`.
   - Click `Select area`, then drag over the subtitle area in the game window.
   - Click `Start`.

If the captured area is wrong, click `Select area` again or manually adjust the subtitle crop ratios near the bottom of the window.
...
Use this when you want online translation with the OpenAI API, including GPT and Codex-family models.
...
Current built-in OpenAI model options:
...
### Subtitle Area Selection

`Select area` follows the common OCR translator region-selection workflow: select a game window, show a translucent overlay on top of it, then drag a rectangle around the subtitle area.

- Left-drag: select the OCR subtitle area
- Release left button: save the area
- Right-click or `Esc`: cancel selection

The selected rectangle is saved as `Left / Top / Right / Bottom` ratios relative to the game window. This means the region still works after moving the game window, as long as the window size stays the same.
